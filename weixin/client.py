...
import time
import requests
from weixin.utils import to_text
import json
...
class Client(object):
    ...
    """
    def __init__(self, appid, appsecret, cfgname, cfgobj):
    ...
    def read_from_cfg(self):
        token = self.cfgobj.get("SYS", "access_token")
        expires = self.cfgobj.get("SYS", "access_token_expires_at")
        return token, int(expires)

    def write_to_cfg(self, _token, token_expires_at):
        self.cfgobj.set("SYS", "access_token", _token)
        self.cfgobj.set("SYS", "access_token_expires_at",\
                        str(token_expires_at))
        with open(self.cfgname, "wb") as f1:
            self.cfgobj.write(f1)

    @property
    def token(self):
        #首次运行时，先从文件取
        if not self._token:
            token, expire = self.read_from_cfg()
            self._token, self.token_expires_at = token, expire
        if self._token:
            now = time.time()
        ...
        #从远端取了后，写入文件
        self.write_to_cfg(self._token, self.token_expires_at)
        return self._token
    ...
    def delivernotify(self, **params):
        '''
        @summary: 发货通知 ?access_token=xxxxxx
        '''
        return self.post(
